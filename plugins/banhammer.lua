...
local function run(msg, matches)
 if matches[1]:lower() == 'id' then
  ...
  if matches[1]:lower() == 'sik' then-- /ban 
...
if matches[1]:lower() == 'sik' then
  ...
  if matches[1]:lower() == 'supersik' then -- Global ban
      ...
      if string.match(targetuser, '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	banall_user(targetuser)
       		return 'User ['..user_id..' ] super siked'
      else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'banall',
  ...
  if matches[1]:lower() == 'unsupersik' then -- Global unban
    local user_id = matches[2]
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then 
  ...
  if matches[1]:lower() == "supersiklist" then -- supersik list
    return banall_list()
  end
end
